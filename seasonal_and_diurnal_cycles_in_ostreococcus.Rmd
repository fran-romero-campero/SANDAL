...
write.table(x = subset(gene.annotation, geneID %in% rhythmic.genes.ld.12),file="gene_sets/LD_2_peaks_genes.tsv",row.names = F)
...
write.table(x = subset(gene.annotation, geneID %in% rhythmic.genes.sd.12),file="gene_sets/SD_2_peaks_genes.tsv",row.names = F)
...
length(genes.two.peaks.ld.one.peak.ll)
...
genes.two.peaks.ld.one.peak.dd.ll <- intersect(genes.two.peaks.ld.one.peak.dd, genes.two.peaks.ld.one.peak.ll)
length(genes.two.peaks.ld.one.peak.dd.ll)
```
...
plot.expression.profile(gene.id ="ostta08g01150", gene.name = "", 
                        gene.expression,entrainment="LD",
...
bimodal.gene.model(gene.id = "ostta03g03980",gene.name = "PA200")
bimodal.gene.model(gene.id = "ostta05g03730",gene.name = "PAS2")
bimodal.gene.model(gene.id = "ostta10g02350",gene.name = "SSS?")
bimodal.gene.model(gene.id = "ostta01g00730",gene.name = "???") #ok
bimodal.gene.model(gene.id = "ostta01g02510",gene.name = "???") #ok
bimodal.gene.model(gene.id = "ostta01g02520",gene.name = "???") #ok
bimodal.gene.model(gene.id = "ostta01g02845",gene.name = "???") #ok?
bimodal.gene.model(gene.id = "ostta14g02390",gene.name = "???") #ok
bimodal.gene.model(gene.id = "ostta11g02060",gene.name = "???") #ok
bimodal.gene.model(gene.id = "ostta08g01440",gene.name = "???") #ok

bimodal.gene.model(gene.id = "ostta07g03850",gene.name = " ")
bimodal.gene.model(gene.id = "ostta06g02730",gene.name = "PIP5K1")
bimodal.gene.model(gene.id = "ostta10g02350",gene.name = " ")

genes.two.peaks.ld.one.peak.dd.ll
write.table(x = subset(gene.annotation, geneID %in% genes.two.peaks.sd.one.peak.ll.dd),file="gene_sets/genes_two_peaks_sd_one_peak_dd_ll.tsv",row.names = F)

genes.two.peaks.sd.one.peak.ll.dd.atha <- subset(ostta2atha, ostta %in% genes.two.peaks.sd.one.peak.ll.dd)$atha


library(clusterProfiler)
library(org.At.tair.db)
enrich.genes.two.peaks.sd.one.peak.ll.dd.ath <- enrichGO(gene = genes.two.peaks.sd.one.peak.ll.dd.atha,
                                          OrgDb = org.At.tair.db,
                                          ont = "BP",
                                          pAdjustMethod = "BH",
                                          pvalueCutoff  = 0.05,
                                          keyType = "TAIR")
as.data.frame(enrich.genes.two.peaks.sd.one.peak.ll.dd.ath)

write(x = genes.two.peaks.sd.one.peak.ll.dd.atha, file="gene_sets/genes_two_peaks_sd_one_peak_dd_ll_atha.tsv")
...
model.deviations[model.deviations > 12] <- 24 - model.deviations[model.deviations > 12]

hist(model.deviations,breaks=0:12,main="",xlab="Model deviation (h)",ylab="Number of Genes",cex.lab=1.5,cex.axis=1.3)
...
dynamic.transition.sd.ld <- function(gene.id, gene.name)
{
 ld.zt <- paste("ld",paste0("zt",sprintf(fmt = "%02d",seq(from=0,to=20,by=4))),sep="_")
 current.gene.expression.ld <- gene.expression[gene.id,c(paste(ld.zt,1,sep="_"),paste(ld.zt,2,sep="_"),paste(ld.zt,3,sep="_"))]
...
dynamic.transition.sd.ld(gene.id = "ostta08g03680",gene.name = "POLAB")
dynamic.transition.sd.ld(gene.id = "ostta01g02580",gene.name = "MCM6")
dynamic.transition.sd.ld(gene.id = "ostta06g02890",gene.name = "PCNA")

dynamic.transition.sd.ld(gene.id = "ostta04g00770",gene.name = "Utp14")
dynamic.transition.sd.ld(gene.id = "ostta03g03015",gene.name = "YeiP")



dynamic.transition.sd.ld(gene.id = "ostta01g03170",gene.name = "PsbP")
dynamic.transition.sd.ld(gene.id = "ostta05g04560",gene.name = "PsbR")



dynamic.transition.sd.ld(gene.id = "ostta06g02940",gene.name = "GBSSI")
...
plot.ld.nd.sd <- function(gene.id, gene.name, ld.gene.expression,nd.gene.expression,sd.gene.expression,ld=T,sd=T,nd=T,scale=T)
{
  ...
  if(nd)
  {
  lines(x = seq(from=0,by=3,to=69),current.gene.expression.nd,type="o",lwd=3,col="darkorchid4")
}
  ...
  if(nd)
  {
    polygon(x = c(24*i, 24*i+12, 24*i+12, 24*i),
            y=c(min.expr-current.line, min.expr-current.line, min.expr-(current.line+0.25), min.expr-(current.line +0.25)),
            lwd=2,border="darkorchid4")
    polygon(x = c(24*i+12,24*(i+1),24*(i+1),24*i+12),
            y=c(min.expr-current.line, min.expr-current.line, min.expr-(current.line+0.25), min.expr-(current.line +0.25)),
            lwd=2,border="darkorchid4",col="darkorchid4")
    current.line <- current.line + 0.5
  }
...
plot.ld.nd.sd(gene.id = "ostta01g00730",gene.name = "", ld.gene.expression,nd.gene.expression,sd.gene.expression,ld=F,sd=F,nd=T,scale=F)
...
write.table(x = ld.protein.abundance,file = "swath_proteomic_data/LD_proteomic_data.tsv",quote = F,sep = "\t")

ld.protein.abundance <- read.table(file="swath_proteomic_data/LD_proteomic_data.tsv",header = T,sep = "\t")
...
plot.ld.gene.prot <- function(gene.id, gene.name, gene.expression, protein.data)
{
  ...
  current.protein.ld <-unlist(2^protein.data[gene.id,c(paste(zts,"1",sep="_"),paste(zts,"2",sep="_"),paste(zts,"3",sep="_"))])
...
circacompare.ld.gene.protein["ostta11g00910",]
...
```{r ld_gene_vs_protein_only_rhythmic}
circacompare.ld.gene.protein <- matrix(nrow=length(ld.protein.ids),ncol=15)
rownames(circacompare.ld.gene.protein) <- ld.protein.ids
...
write.table(x = sd.protein.abundance,file = "swath_proteomic_data/SD_proteomic_data.tsv",quote = F,sep = "\t")
...
plot.sd.gene.prot <- function(gene.id, gene.name, gene.expression, protein.data)
{
  ...
  zts <- paste("zt",seq(from=0,to=20,by=4),sep="")
  current.protein.sd <- unlist(2^protein.data[gene.id,c(paste(zts,"1",sep="_"),paste(zts,"2",sep="_"),paste(zts,"3",sep="_"))])
...
circacompare.sd.gene.protein["ostta11g00910",]
...
names(G2.day2.LD) <- time.points[7:12]
...
S.day1.SD <- S.percentage.SD[1:6]
names(S.day1.SD) <- time.points[1:6]
S.day2.SD <- S.percentage.SD[7:12]
names(S.day2.SD) <- time.points[7:12]
S.day3.SD <- S.percentage.SD[13:18]
names(S.day3.SD) <- time.points[13:18]
S.day4.SD <- S.percentage.SD[19:24]
...
sd.protein.abundance <- read.table(file="swath_proteomic_data/SD_proteomic_data.tsv",header = T,sep = "\t")
...
ld.cyca.expression <- ld.mean.gene.expression["ostta02g00150",]
ld.cycb.expression <- ld.mean.gene.expression["ostta01g06150",]
ld.cycd.expression <- ld.mean.gene.expression["ostta18g01570",]
#ld.cycd.expression <- ld.mean.gene.expression["ostta18g01420",]

#ld.cyca.expression.sem <- ld.sem.gene.expression["ostta18g01570",]
ld.cyca.expression.sem <- ld.sem.gene.expression["ostta02g00150",]
ld.cycb.expression.sem <- ld.sem.gene.expression["ostta01g06150",]
ld.cycd.expression.sem <- ld.sem.gene.expression["ostta18g01570",]
...
sd.cyca.expression <- sd.mean.gene.expression["ostta02g00150",]
sd.cycb.expression <- sd.mean.gene.expression["ostta01g06150",]
sd.cycd.expression <- sd.mean.gene.expression["ostta18g01570",]
...
sd.cyca.expression.sem <- sd.sem.gene.expression["ostta02g00150",]
sd.cycb.expression.sem <- sd.sem.gene.expression["ostta01g06150",]
sd.cycd.expression.sem <- sd.sem.gene.expression["ostta18g01570",]
...
lines(x = seq(from=0,to=24,by=4),y = c(ld.current.protein.abundance,ld.current.protein.abundance[1]),
      lwd=3, col="black",type="o",pch=20,cex=1.5)
arrows(x0 = seq(from=0,to=24,by=4),y0 =  c(ld.current.protein.abundance,ld.current.protein.abundance[1]) - c(scaled.sem.ld.protein.abundance[gene,],scaled.sem.ld.protein.abundance[gene,1]),x1 = seq(from=0,to=24,by=4), y1 = c(ld.current.protein.abundance,ld.current.protein.abundance[1]) + c(scaled.sem.ld.protein.abundance[gene,],scaled.sem.ld.protein.abundance[gene,1]),length = 0.05,angle = 90,code = 3,col = "black",lwd=3)
axis(side=2,lwd=3,at = seq(from=-2,to=2,by=1),labels=rep("",5),cex.axis=1.2,las=2)
...
ld.fvfm.values <- ld.fvfm.data$fvfm
names(ld.fvfm.values) <- ld.fvfm.data$time_point

rain(x = ld.fvfm.values[sorted.zts],deltat = 4,nr.series = 3,period = 24)
rain(x = ld.fvfm.values[sorted.zts],deltat = 4,nr.series = 3,period = 12)

sd.fvfm.data <- read.table(file = "physiological_data/photosynthesis/Fv_Fm_SD.tsv",sep="\t",as.is=T,header=T)

sd.fvfm.values <- sd.fvfm.data$fvfm
names(sd.fvfm.values) <- sd.fvfm.data$time_point

rain(x = sd.fvfm.values[sorted.zts],deltat = 4,nr.series = 3,period = 24)
rain(x = sd.fvfm.values[sorted.zts],deltat = 4,nr.series = 3,period = 12)

library(circacompare)
fvfm.time.points <- seq(from=0,by=4,length.out = 18)
circacompare.fvfm <- data.frame(time=c(fvfm.time.points,
                                       fvfm.time.points),
                                measure=c(ld.fvfm.values,
                                          sd.fvfm.values),
                                group=c(rep("LD",18),rep("SD",18)))

result.fvfm <- circacompare(x = circacompare.fvfm, 
                              col_time = "time", 
                              col_group = "group", 
                              col_outcome = "measure",
                              alpha_threshold = 1)
result.fvfm.values <- result.fvfm$summary$value
names(result.fvfm.values) <- result.fvfm$summary$parameter


ld.fvfm.max.val <- (0.6065163 + 0.6091954 + 0.6112469)/3
sd.fvfm.max.val <- (0.5597190 + 0.5621891 + 0.5773710)/3

(ld.fvfm.max.val - sd.fvfm.max.val)/ld.fvfm.max.val * 100

ld.fvfm.min.val <- (0.5723270 + 0.5612903 + 0.5769231)/3
sd.fvfm.min.val <- (0.4528986 + 0.4586207 + 0.4669118)/3
(ld.fvfm.min.val - sd.fvfm.min.val)/ld.fvfm.min.val * 100
...
starch.content.ld.max <- (41.87 + 40.46)/2
starch.content.sd.max <- (33.65 + 33.11)/2

100*(starch.content.ld.max - starch.content.sd.max)/starch.content.ld.max
...
nr.activity.ld <- enzymatic.activity$NR[enzymatic.activity$Condition == "LD"]
names(nr.activity.ld) <- enzymatic.activity$Time[enzymatic.activity$Condition == "LD"]

nr.ld.results <- rain(nr.activity.ld[c("ZT0_1","ZT0_2","ZT4_1","ZT4_2","ZT8_1","ZT8_2",
"ZT12_1","ZT12_2","ZT16_1","ZT16_2","ZT20_1","ZT20_2")], deltat=4, period=24, verbose=FALSE, nr.series=2)

nr.activity.sd <- enzymatic.activity$NR[enzymatic.activity$Condition == "SD"]
names(nr.activity.sd) <- enzymatic.activity$Time[enzymatic.activity$Condition == "SD"]
nr.sd.results <- rain(nr.activity.sd[c("ZT0_1","ZT0_2","ZT4_1","ZT4_2","ZT8_1","ZT8_2",
"ZT12_1","ZT12_2","ZT16_1","ZT16_2","ZT20_1","ZT20_2")], deltat=4, period=24, verbose=FALSE, nr.series=2)


library(circacompare)
time.points <- seq(from=0,by=4,length.out = 12)
circacompare.data <- data.frame(time=c(time.points,
                                       time.points),
                                measure=c(nr.activity.ld,
                                          nr.activity.sd),
                                group=c(rep("LD",12),rep("SD",12)))

result.nr <- circacompare(x = circacompare.data, 
                              col_time = "time", 
                              col_group = "group", 
                              col_outcome = "measure",
                              alpha_threshold = 1)
result.nr.values <- result.nr$summary$value
names(result.nr.values) <- result.nr$summary$parameter

gs.activity.ld <- enzymatic.activity$GS[enzymatic.activity$Condition == "LD"]
names(gs.activity.ld) <- enzymatic.activity$Time[enzymatic.activity$Condition == "LD"]

gs.ld.results <- rain(gs.activity.ld[c("ZT0_1","ZT0_2","ZT4_1","ZT4_2","ZT8_1","ZT8_2",
"ZT12_1","ZT12_2","ZT16_1","ZT16_2","ZT20_1","ZT20_2")], deltat=4, period=24, verbose=FALSE, nr.series=2)

gs.activity.sd <- enzymatic.activity$GS[enzymatic.activity$Condition == "SD"]
names(gs.activity.sd) <- enzymatic.activity$Time[enzymatic.activity$Condition == "SD"]
gs.sd.results <- rain(gs.activity.sd[c("ZT0_1","ZT0_2","ZT4_1","ZT4_2","ZT8_1","ZT8_2",
"ZT12_1","ZT12_2","ZT16_1","ZT16_2","ZT20_1","ZT20_2")], deltat=4, period=24, verbose=FALSE, nr.series=2)
...
circacompare.data <- data.frame(time=c(time.points,
                                       time.points),
                                measure=c(gs.activity.ld,
                                          gs.activity.sd),
                                group=c(rep("LD",12),rep("SD",12)))

result.gs <- circacompare(x = circacompare.data, 
                              col_time = "time", 
                              col_group = "group", 
                              col_outcome = "measure",
                              alpha_threshold = 1)
result.gs.values <- result.gs$summary$value
names(result.nr.values) <- result.nr$summary$parameter
